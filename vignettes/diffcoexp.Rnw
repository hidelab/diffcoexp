...
b). The differences between the correlation coefficients under two conditions are
calculated and their p values are calculated using Fisher's Z-transformation.

c). p values are adjusted.

d). Gene pairs (links) coexpressed in at least one condition are identified using the
criteria that at least one of the correlation coefficients under two conditions has
absolute value greater than the threshold \emph{rth} and adjusted p value less than the
...
coefficients is greater than the threshold \emph{r.diffth} and adjusted p value is less than
...
them meets the criteria that absolute correlation coefficient is greater than the
...
conditions and both of them meet the criteria that absolute correlation coefficient
is greater than the threshold \emph{rth} and adjusted p value less than the threshold \emph{qth}.
...
\noindent
From May 2018, this package will be available from Bioconductor and can be installed within R as follows: \par
\#\# try http:// if https:// URLs are not supported \par
source("https://bioconductor.org/biocLite.R") \par
BiocManager::install("diffcoexp") \smallskip
\par
\noindent
...
R CMD check diffcoexp\_0.99.5.tar.gz \par
R CMD INSTALL diffcoexp\_0.99.5.tar.gz \smallskip
\par
\noindent
